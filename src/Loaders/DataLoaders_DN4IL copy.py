...
class DN4IL(Dataset):
    def __init__(self, root, root_dn4il, image_size=384, validation_size=0.15, random_state=42, start_domain_real=True):
        self.root = root
        ...
        self.data = {partition: {domain: {"paths": [], "labels": []} for domain in domains} 
        ...
        for domain in domains:
            ...
            if partition == partition.TRAIN:
                self.data[partition.TRAIN][domain]["paths"] = X_train
                self.data[partition.TRAIN][domain]["labels"] = y_train
                
            elif partition == partition.VALIDATION:
                self.data[partition.VAL][domain]["paths"] = X_val
                self.data[partition.VAL][domain]["labels"] = y_val
            ...
            self.data[partition.TEST][domain]["paths"] = [line.split(' ')[0] for line in lines]
            self.data[partition.TEST][domain]["labels"] = [int(line.split(' ')[1]) for line in lines]
...
class DN4IL_data(DN4IL):
    def __init__(self, data, partition):
        super(DN4IL_data, self).__init__(partition)
    ...
    def __len__(self):
        return len(self.data[self.partition][self.Domain2Use]["labels"])
    
    def __getitem__(self, idx):

        img_path = os.path.join(self.root, self.data[self.partition][self.Domain2Use]["paths"][idx])
        label = self.data[self.partition][self.Domain2Use]["labels"][idx]
...
def get_loaders(path, path_dn4il, image_size=224, batch_size=32):
    ...
    data = DN4IL(path, path_dn4il, partition.TRAIN, image_size=image_size)
    
    train_dataset = DN4IL_data(data, partition.TRAIN)
    val_dataset = DN4IL_data(data, partition.VALIDATION)
    test_dataset = DN4IL_data(data, partition.TEST)

...
teacher = baseline_with_ewc(num_classes=100)
for idx, domain in enumerate(dataset.domains):

    if "Teacher" in yaml_name:
        teacher_weights_path = f"{runs_dir}/{yaml_name}/model_{domain}_teacher.pth"
    else:
        teacher_weights_path = f"{runs_dir}/{yaml_name}/model_{domain}.pth"
        
    teacher_weights = torch.load(teacher_weights_path)
    ...
    for i in range(idx + 1):
        print(f"Domain: {i} with weights: {teacher_weights_path}")
        test_dataloader.dataset.select_domain(i)
        test_top1_acc_d, test_top5_acc_d = top1_and_top_k_accuracy_domain(teacher, test_dataloader, device, k=5)
...
for i in range(len(dataset.domains)):
    test_dataloader.dataset.select_domain(i)
    test_top1_acc, test_top5_acc = top1_and_top_k_accuracy_domain(teacher, test_dataloader, device, k=5)
    result_top1.append(test_top1_acc.cpu())
import os
os.makedirs(f"{runs_dir}/{yaml_name}/EvaluationTeacher", exist_ok=True)
createCSV(test_prev_accs_top1, f"{runs_dir}/{yaml_name}/EvaluationTeacher/afterEachdomain_top1_plasticity.csv", idx2domain)
createCSV(test_prev_accs_top5, f"{runs_dir}/{yaml_name}/EvaluationTeacher/afterEachdomain_top5_plasticity.csv", idx2domain)

plotStablityPlasticity(test_prev_accs_top1, result_top1, f"{runs_dir}/{yaml_name}/EvaluationTeacher/stability_plasticity.png")
plot_strictly_lower_triangular_heatmap(test_prev_accs_top1, list(idx2domain.values()), f"{runs_dir}/{yaml_name}/EvaluationTeacher/heatmap_top1.png")
plot_strictly_lower_triangular_heatmap(test_prev_accs_top5, list(idx2domain.values()), f"{runs_dir}/{yaml_name}/EvaluationTeacher/heatmap_top5.png")

...
def compute_domain_similarities(domain_histograms, domain_names):
    ...
    for i in range(num_domains):
        for j in range(num_domains):
            if i <= j:  # Compute only upper triangle and mirror
                ...
                emd_values = []
                for hist_i in hists_i:
                    for hist_j in hists_j:
                        emd = wasserstein_distance(hist_i, hist_j)
                        emd_values.append(emd)
                avg_emd = np.mean(emd_values)
                similarity_matrix[i, j] = avg_emd
                similarity_matrix[j, i] = avg_emd  # Symmetric matrix
        print(f"Computed similarities for domain {domain_names[i]}.")

    return similarity_matrix

def save_similarity_matrix_plot(matrix, domains, save_path="domain_similarity_matrix.png"):
    plt.figure(figsize=(8, 6))
    plt.imshow(matrix, interpolation='nearest', cmap='viridis')
    plt.title('Domain Similarity Matrix (Average EMD)')
    plt.colorbar(label='Average EMD')
...
def main():
    ...
    domains = ['clipart', 'quickdraw']

    domain_histograms = {}
    num_bins = 64  # Number of bins for histograms
    sample_size = 100 # Number of images to sample for histogram computation
    ...
    similarity_matrix = compute_domain_similarities(domain_histograms, domains)

    print("\nDomain Similarity Matrix (Average EMD):")
    print("Domains:", domains)
    print(similarity_matrix)

    # Save the similarity matrix plot
    save_similarity_matrix_plot(similarity_matrix, domains, save_path="domain_similarity_matrix2.png")
